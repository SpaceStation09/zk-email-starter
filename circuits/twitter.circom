...
template Twitter(max_header_bytes, max_body_bytes, n, k, pack_size, expose_from, expose_to){
  assert(expose_from < 2); //boolean input: 1 if we should expose the from, 0 if we should not
  assert(expose_to == 0); // 1 if we should expose the to, 0 if we should not: due to hotmail restrictions, we force-disable this
  
  // prehashed email data, includes up to 512 + 64? bytes of padding pre SHA256, and padded with lots of 0s at end after the length
  signal input in_padded[max_body_bytes];
  ...
  // length of in email data including the padding, which will inform the sha256 block length
